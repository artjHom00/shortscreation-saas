let { verifyCryptoCloudToken } = require('../providers/jwt')
...
async function handleNotification(req, res) {
    try {
        let { status, invoice_id, amount_crypto, currency, order_id, token } = req.body
        ...
        if(transaction) {
            ...
            res.status(500).send({ error: 'Transaction not found' })
        }
...
async function createTransaction(req, res) {
    try {
        const { user_id, type, order_id, amount } = req.body;
